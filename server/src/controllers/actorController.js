...
export async function getActor (req, res) {
  ...
  const resActor = await Actor.findOne({ _id: actor, organisation, account, public: isPublic });
  return res.json({ actor: resActor });
}

export function currentlyNotSupported (req, res) {
  return res.status(404).json({ error: `The path '${req.route.path}' is currently not supported` });
}
